...
payload (void)
{
  ...
  while (count > 0) {
    ...
    if ((!dev_type) || (dev_type > 1)) {
      val_print (AVS_PRINT_DEBUG, "\n       Skipping PCIe host bridge and PCIe bridge devices", 0);
      continue;
    }

    if (val_pcie_get_dma_support (dev_bdf) == 1) {
      val_print (AVS_PRINT_INFO, "\n       have DMA support on %X", dev_bdf);
      if (val_pcie_get_dma_coherent(dev_bdf) == 1) {
        val_print (AVS_PRINT_INFO, "\n       DMA is coherent on %X", dev_bdf);
        status = val_pcie_get_snoop_bit (dev_bdf);
        if (status != 2) {
          no_snoop_set |= status;
          val_print (AVS_PRINT_INFO, "\n       no snoop bit is %d", status);
        }
      } else {
        val_print (AVS_PRINT_INFO, "\n       DMA is not coherent on %X", dev_bdf);
      }
    }
  }
