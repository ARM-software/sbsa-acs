      ...
      if (dp_type == RCiEP) {
          ...
          offset = BAR0_OFFSET;
           ...
           while (offset <= BAR_TYPE_1_MAX_OFFSET) {
              ...
              {
                  val_print(AVS_PRINT_INFO, "The BAR supports 64-bit address capability \n", 0);
                ...
                if (bar_orig == bar_new) {
                    val_print(AVS_PRINT_TEST, "Read write to BAR reg not supported bdf %x\n", bdf);
                    fail_cnt++;
                }
                 ...
                 val_print(AVS_PRINT_INFO, "The BAR supports 32-bit address capability\n", 0);
