...
#define TEST_NUM (AVS_MPAM_TEST_NUM_BASE + 5)
    ...
    for (msc_index = 0; msc_index < msc_node_cnt; msc_index++) {
        ...
        for (msc_index1 = msc_index + 1; msc_index1 < msc_node_cnt; msc_index1++) {
            ...
            if (msc_addr1 >= msc_addr && msc_addr1 <= (msc_addr + msc_len)) {
                ...
                val_print(AVS_PRINT_ERR, " %d memory layout overlapping", msc_index1);
                test_fails++;
            }
        }
        ...
        while (peri_count) {
            ...
            if (peripheral_base >= msc_addr && peripheral_base <= (msc_addr + msc_len)) {
                val_print(AVS_PRINT_ERR, "\n       MSC %d and", msc_index);
                val_print(AVS_PRINT_ERR, " USB %d memory layout overlapping", peri_count);
                test_fails++;
            }
        }
        ...
        while (peri_count) {
            ...
            if (peripheral_base >= msc_addr && peripheral_base <= (msc_addr + msc_len)) {
                val_print(AVS_PRINT_ERR, "\n       MSC %d and", msc_index);
                val_print(AVS_PRINT_ERR, " UART %d memory layout overlapping", peri_count);
                test_fails++;
            }
        }
        ...
        while (peri_count) {
            ...
            if (peripheral_base >= msc_addr && peripheral_base <= (msc_addr + msc_len)) {
                val_print(AVS_PRINT_ERR, "\n       MSC %d and", msc_index);
                val_print(AVS_PRINT_ERR, " SATA %d memory layout overlapping", peri_count);
                test_fails++;
            }
        }
    }
