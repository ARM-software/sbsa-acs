...
#define TEST_NUM   (AVS_EXERCISER_TEST_NUM_BASE + 19)
...
payload(void)
{
  ...
  while (instance-- != 0)
  {
      ...
      status = val_pcie_find_capability(e_bdf, PCIE_ECAP, ECID_DPC, &dpc_cap_base);
      test_skip = 0;
      ...
      for (int i = 0; i < 2; i++)
      {
          ...
          val_pcie_read_cfg(e_bdf, dpc_cap_base + DPC_STATUS_OFFSET, &reg_value);
          val_pcie_write_cfg(e_bdf, dpc_cap_base + DPC_STATUS_OFFSET, reg_value | 0x1);
