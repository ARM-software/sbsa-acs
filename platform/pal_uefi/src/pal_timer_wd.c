...
pal_timer_create_info_table(TIMER_INFO_TABLE *TimerTable)
{
  ...
  EFI_ACPI_6_1_GTDT_GT_BLOCK_TIMER_STRUCTURE *GtBlockTimer;
  TIMER_INFO_GTBLOCK         *GtEntry;
  UINT32                      Length= 0;
  UINT32                      i;
  ...
  TimerTable->header.CntControl_base    = gGtdtHdr->CntControlBasePhysicalAddress;
  TimerTable->header.CntRead_base       = gGtdtHdr->CntReadBasePhysicalAddress;
...
pal_wd_create_info_table(WD_INFO_TABLE *WdTable)
{
  ...
  EFI_ACPI_6_1_GTDT_SBSA_GENERIC_WATCHDOG_STRUCTURE    *Entry;
  WD_INFO_BLOCK               *WdEntry = WdTable->wd_info;
  UINT32                      Length= 0;
  UINT32                      num_of_entries;
